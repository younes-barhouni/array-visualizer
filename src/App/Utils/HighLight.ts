...
export default class Highlight {
    ...
    constructor(index, height, container, data, dataWidths, parensWidth, commaWidth) {
        this.index = index;
        this.height = height ;
    ...
    private init() {
        ...
        this.rect = this.container.append("rect")
            ...
            .attr('opacity', 0)
            .attr("transform","translate("+(this.targetX - this.padding)+ "," + (this.targetY - this.padding) + ")");
        this.updateAll();
    }

    private calculate(){
        this.targetX = d3.sum(this.dataWidths.slice(0, this.index)) + this.parensWidth + this.index * this.commaWidth;
